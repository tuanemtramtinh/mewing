...
function changeWidth(){
    ...
    if(diff_Now__end <= 0){
        ...
        progress.innerHTML = `<i class="material-icons" style="align-items:center">local_shipping</i>`;
    }
    else{
        progress.style.width = `100%`;
        progress.innerHTML = `<i class="material-icons">done</i>`;
    }
}
    ...
    getDoc(carRef)
        .then((doc) => {
            if(doc.data()){
                ...
                timeEnd = moment(`${doc.data().departureDate} ${doc.data().arriveTime}`, "YYYY-MM-DD HH:mm:ss");
